...
variable "terraform_locks_table_billing_mode" {
  description = "Billing mode for the table (PROVISIONED or PAY_PER_REQUEST)"
  type        = string
  default     = "PAY_PER_REQUEST"
}

variable "terraform_locks_table_hash_key" {
  description = "Partition key attribute name"
  type        = string
  default     = "LockID"
}

variable "terraform_locks_table_range_key" {
  description = "Sort key attribute name"
  type        = string
  default     = null
}

variable "terraform_locks_table_attributes" {
...
variable "terraform_locks_table_read_capacity" {
  description = "Read capacity units for PROVISIONED billing mode"
  type        = number
  default     = 5
}

variable "terraform_locks_table_write_capacity" {
  description = "Write capacity units for PROVISIONED billing mode"
  type        = number
  default     = 5
}

variable "terraform_locks_table_class" {
  description = "Storage class of the table (STANDARD or STANDARD_INFREQUENT_ACCESS)"
  type        = string
  default     = "STANDARD"
}

variable "terraform_locks_table_deletion_protection_enabled" {
  description = "Enable deletion protection for the table"
  type        = bool
  default     = false
}

variable "terraform_locks_table_stream_enabled" {
  description = "Enable DynamoDB Streams"
  type        = bool
  default     = false
}

variable "terraform_locks_table_stream_view_type" {
  description = "When stream_enabled is true, stream view type (KEYS_ONLY, NEW_IMAGE, OLD_IMAGE, NEW_AND_OLD_IMAGES)"
  type        = string
  default     = "NEW_AND_OLD_IMAGES"
}

variable "terraform_locks_table_server_side_encryption_enabled" {
  description = "Enable server-side encryption"
  type        = bool
  default     = true
}

variable "terraform_locks_table_kms_key_arn" {
  description = "KMS key ARN for table encryption (null uses AWS owned key)"
  type        = string
  default     = null
}

variable "terraform_locks_table_point_in_time_recovery_enabled" {
  description = "Enable point-in-time recovery"
  type        = bool
  default     = true
}

variable "terraform_locks_table_ttl_enabled" {
  description = "Enable TTL for the table"
  type        = bool
  default     = false
}

variable "terraform_locks_table_ttl_attribute_name" {
  description = "TTL attribute name (required when ttl_enabled is true)"
  type        = string
  default     = null
}

variable "terraform_locks_table_local_secondary_indexes" {
...
variable "terraform_locks_table_global_secondary_indexes" {
...
variable "terraform_locks_table_tags" {
